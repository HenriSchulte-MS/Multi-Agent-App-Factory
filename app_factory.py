...
async def get_agents(client, session_dir: str) -> list[Agent]:
    # Developer agent
    ...
    qa_agent_definition = await client.agents.create_agent(
        ...
        instructions="You are an excellent quality assurance specialist. You create and execute test cases to ensure the quality of web applications. You do not interfere with the human expert review.",
        description="A quality assurance specialist.",
    )
    qa_agent = AzureAIAgent(
        client=client,
        definition=qa_agent_definition,
        plugins=[TestPlugin()]  # Register the TestPlugin for the QA agent
    )
    ...
    return [dev_agent, qa_agent, calling_agent]
...
async def main() -> None:
    ...
    async with (
        ...
        # Get agents with their definitions created
        agents = await get_agents(client, session_dir)
        ...
        try:
            ...
            orchestration_result = await group_chat_orchestration.invoke(
                task=(
                        ...
                        "Task: Create a web application that is a scientific calculator. It must be named Microsoft Scientific Calculator."
                        ...
                        "4. Ensure all requirements named in the task description have been met and that a human expert review has been passed."
